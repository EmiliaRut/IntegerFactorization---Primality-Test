...
public class GA {
    ...
    private long averageEval;
    
  public GA(long n, int cross, int mut) {
    N = n;
    CROSS_RATE = cross;
    MUT_RATE = mut;
    ...
    if (length % 2 == 0) {
        chromLength = length/2 - 2; //subtract 2 to get chromosome length that is divided by 6
    } else {
        chromLength = (length + 1)/2 - 3; //subtract 2 to get chromosome length that is divided by 6
    }
...
//    System.out.println("----------------------------- next gen");
  ...
  public void elitism() {    
    ...
    for(int i = 0; i < POP_SIZE; i++) {
      if(pop.get(worstIndex).getEval() < pop.get(i).getEval()) {
        worstIndex = i;
      }
    }
  ...
  public void crossover(Chromosome p1, Chromosome p2) {  //ordered crossover
    ...
    while(time < chromLength*2) {
        ...
        if (child1.isPrime() && child2.isPrime()) break;
    }
  ...
  public void findAverage() {
    ...
    averageEval = sum / POP_SIZE;
//    System.out.println("Average Eval: " + averageEval);
  } //findAverage()
  
  public long getAverageEval() {
      return averageEval;
  }//getAverageEval()
...
//    fittestChrom.setPrime(pop.get(0).getPrime());
    ...
    for(int i = 0; i < POP_SIZE; i++) {
      if(fittestChrom.getEval() > pop.get(i).getEval()) {
